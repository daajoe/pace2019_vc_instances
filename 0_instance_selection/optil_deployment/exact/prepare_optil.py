...
CONFIGS = {
    'main': {
        'input': 'full',
        'output': '../../../../../TestData/selection/vc/main/output'    
    }
}
...
def main():
    for key in CONFIGS.keys():
        ...
        for idx in range(1, INSTANCE_COUNT+1):
            ...
            input_file = os.path.join(config['input'], "vc-exact_{}.gr".format(pretty_idx))
            output_file = os.path.join(config['output'], instance_type, "vc-exact_{}.out".format(pretty_idx))
            
            dest_input_file = os.path.join(root, instance_type, "vc-exact_{}.in".format(pretty_idx))
            dest_output_file = os.path.join(root, instance_type, "vc-exact_{}.out".format(pretty_idx))
